...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def __init__(self):
        ...
        self._actions = {
            'xiaomi_account': "Login with cloud",
    ...
    async def async_step_user(self, user_input=None, non_interactive=False):   # 1. 选择操作
        ...
        if user_input is not None:
            ...
            elif user_input['action'] == 'accesstoken':
                return await self.async_step_accesstoken()
            else:
    ...
    async def async_step_accesstoken(self, user_input=None):
        """Handle User token"""
        if user_input:
            if 'userid' in user_input:
                ...
                reqtoken = cloud.login_by_credientals(user_input['userid'], user_input['serviceToken'], user_input['ssecurity'])
                ...
                if reqtoken == (0, None):
                    ...
                    if not self._non_interactive:
                        self.hass.async_add_job(self.hass.config_entries.flow.async_init(
                            DOMAIN, 
                            context={"source": "user"}, 
                            data={'action': 'accesstoken', 'userId': user_input['userid'],'serviceToken': user_input['serviceToken'],'ssecurity': user_input['ssecurity']}
                        ))
                        return await self.async_step_select_devices()
                    else:
                        return self.async_create_entry(title=data_masking(user_input['userid'], 4),
                                data=user_input)
        ...
        return self.async_show_form(
            step_id='accesstoken', data_schema=vol.Schema(data_schema)
        )
                ...
                if resp == (0, None):
                    ...
                    if not self._non_interactive:
                        self.hass.async_add_job(self.hass.config_entries.flow.async_init(
                            DOMAIN, 
                            context={"source": "user"}, 
                            data={'action': 'xiaomi_account', 'username': user_input['username'],'password': user_input['password'],'server_location': user_input['server_location']}
                        ))
                        return await self.async_step_select_devices()
                    else:
